...
namespace JorisHoef.PackageInstaller.Editor.Tests
{
    internal sealed class PackageRegistryTests
    {
        private const string ValidRegistryJson =
            ...
            "{ \"id\": \"com.jorishoef.core.state\", \"displayName\": \"JorisHoef Core State\", \"category\": \"Core\", \"description\": \"Core package.\", \"stableUrl\": \"https://github.com/JorisHoef/Core-State.git#main\", \"developmentUrl\": \"https://github.com/JorisHoef/Core-State.git#develop\", \"dependencies\": [] }," +
            "{ \"id\": \"com.jorishoef.core-state.bridge\", \"displayName\": \"Core Bridge\", \"category\": \"Bridge\", \"description\": \"Bridge package.\", \"stableUrl\": \"https://github.com/JorisHoef/Core-State-Bridge.git#main\", \"developmentUrl\": \"https://github.com/JorisHoef/Core-State-Bridge.git#develop\", \"dependencies\": [\"com.jorishoef.core.state\"] }" +
            "] }";
        ...
        public void PackageNameValidationRejectsPackageIdMismatch()
        {
            PackageRegistry registry = new PackageRegistry
            {
                ...
                packages = new[]
                {
                    new PackageRegistryEntry
                    {
                        ...
                        displayName = "JorisHoef Core State",
                        category = "Core",
                        stableUrl = "https://github.com/JorisHoef/Core-State.git#main",
                        dependencies = Array.Empty<string>()
                    }
                }
            };
            ...
            bool isValid = PackageRegistryPackageNameValidator.ValidatePackageNames(
                registry,
                _ => "{ \"name\": \"com.jorishoef.core.state\" }",
                out string message);
        ...
        [Test]
        public async Task RemoteRegistryPackageNameMismatchKeepsBundledRegistry()
        {
            PackageRegistry bundledRegistry = new PackageRegistry
            {
                schemaVersion = 1,
            ...
            string remoteJson =
                "{ \"schemaVersion\": 1, \"packages\": [" +
                "{ \"id\": \"com.jorishoef.core-state\", \"displayName\": \"JorisHoef Core State\", \"category\": \"Core\", \"stableUrl\": \"https://github.com/JorisHoef/Core-State.git#main\", \"dependencies\": [] }" +
                "] }";

            PackageRegistryLoader loader = new PackageRegistryLoader(
                _ => Task.FromResult(remoteJson),
                packageManifestFetcher: _ => Task.FromResult("{ \"name\": \"com.jorishoef.core.state\" }"));
        ...
        public void GitHubPackageJsonUrlUsesBranchAndPackagePath()
        {
            bool resolved = PackageRegistryPackageNameValidator.TryCreateGitHubPackageJsonUrl(
                "https://github.com/JorisHoef/Example.git?path=/Packages/Example#develop",
                out string packageJsonUrl);

            Assert.IsTrue(resolved);
            Assert.AreEqual(
                "https://raw.githubusercontent.com/JorisHoef/Example/develop/Packages/Example/package.json",
                packageJsonUrl);
        }
        ...
        public void BundledRegistryUsesRealCoreStatePackageId()
        {
            ...
            PackageDefinition coreState = PackageRegistryProvider
                .CreatePackageDefinitions(result.Registry)
                .Single(package => package.DisplayName == "JorisHoef Core State");
            ...
            Assert.AreEqual("com.jorishoef.core.state", coreState.PackageId);
            Assert.IsFalse(dependencyIds.Contains("com.jorishoef.core-state"));
        }
        ...
        public void CoreStateInstalledDetectionUsesRealPackageId()
        {
            ...
            {
                detectionService.ReplaceInstalledPackageNamesForTests(new[] { "com.jorishoef.core.state" });

                Assert.IsTrue(detectionService.IsInstalled(coreState.PackageId));
                Assert.IsFalse(detectionService.IsInstalled("com.jorishoef.core-state"));
            }
        }
